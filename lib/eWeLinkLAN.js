         ...
         if (packet.answers) {
            ...
            .forEach(value => {
               if (value.type === "TXT") {
                  ...
                  let apiKey = this.deviceMap[rdata.id].apiKey;
                  ...
                  let iv = Buffer.from(rdata.iv, 'base64');
                  let cText = Buffer.from(data, 'base64');
                  let dText = crypto.createDecipheriv('aes-128-cbc', key, iv);
                  let pText = Buffer.concat([dText.update(cText), dText.final()]).toString('utf8');
                  let returnTemplate = {
                     action: "update",
                     deviceid: rdata.id,
