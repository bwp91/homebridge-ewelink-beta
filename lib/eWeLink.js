...
class eWeLink {
   ...
   internalGarageUpdate(accessory, value, callback) {
      try {
         ...
         if (!sAccessory) {
            ...
            this.log.warn("This won't actually happen yet so the garage door will continue saying opening/closing.");
   ...
   externalGarageUpdate(accessory, params) {
      try {
         ...
         if (sAccessory) {
            ...
            this.log.warn("This won't actually happen yet so the garage door won't change status in Homebridge.");
   ...
   externalSensorUpdate(accessory, params) {
      try {
         accessory.getService(Service.ContactSensor).updateCharacteristic(Characteristic.ContactSensorState, params.switch === "on" ? 0 : 1);
         this.log("=== EXTERNAL SENSOR UPDATE [ESU] ===");
         this.log("ESU1 sensor has sent update that it is now [%s]", params.switch === "on" ? "joined" : "apart");
         ...
         this.customGroups.forEach(group => {
            if (group.sensorId === accessory.context.eweDeviceId && group.type === "garage") {
               ...
               // if the sensor is used as part of a garage/blind config then:
               if ((oAccessory = this.devicesInHB.get(group.deviceId + "SWX"))) {
                  let currentSensor = params.switch === "on" ? 0 : 1;
                  // 0 = contact detected -> garage closed
                  // 1 = contact not dtcd -> garage open
                  let currentDoorPos = oAccessory.getService(Service.GarageDoorOpener)
                     .getCharacteristic(Characteristic.CurrentDoorState).value;
                  // 0 = garage open
                  // 1 = garage closed
                  if (currentSensor + currentDoorPos === 1) return; // synchronised
                  this.log("ESU2 sensor found linked to [%s]...", oAccessory.displayName);
                  if (params.switch === "on") {
                     ...
                     // ($value MUST BE 1) then sensor has broken contact:
