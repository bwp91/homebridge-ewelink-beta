...
class eWeLink {
   constructor(log, config, api) {
      ...
      this.api.on("didFinishLaunching", () => {
         ...
         //*** Set up HTTP client and get the user's http host ***\\
         ...
         this.httpClient.getHost()
            .then(res => { //*** Use the HTTP API host to log into eWeLink ***\\
               ...
               this.lanClient = new eWeLinkLAN(this.config, this.log);
               return this.lanClient.getHosts();
            }).then(res => { //*** Set up the LAN mode listener
               this.lanDevices = res;
               if (this.debug) {
                  this.log("Future LAN mode support:\n" + JSON.stringify(this.lanDevices, null, 2));
               }
               ...
               (() => {
                  ...
                  if (this.config.groups && Object.keys(this.config.groups).length > 0) {
                     this.config.groups.forEach(group => {
                        if (typeof group.this.deviceId !== "undefined" && this.devicesInEwe.has(group.this.deviceId.toLowerCase())) {
                           this.customGroups.set(group.this.deviceId + "SWX", group);
                        }
                     });
                  }
   ...
   initialiseDevice(device) {
      ...
      if (!this.devicesInHB.has(device.deviceid + "SWX") && !this.devicesInHB.has(device.deviceid + "SW0")) {
         if (this.customGroups.has(device.deviceid + "SWX")) {
            //*** ADD BLINDS ***\\
            if (this.customGroups.get(device.deviceid + "SWX").type === "cusBlind" && Array.isArray(device.params.switches)) {
               this.addAccessory(device, device.deviceid + "SWX", "cusBlind");
            }
            //*** ADD GARAGES ***\\
            else if (this.customGroups.get(device.deviceid + "SWX").type === "cusGarage" && device.params.hasOwnProperty("switch")) {
               this.addAccessory(device, device.deviceid + "SWX", "cusGarage");
            }
         }
         //*** ADD SENSORS ***\\
      ...
      if ((accessory = this.devicesInHB.get(device.deviceid + "SWX"))) {
         if (this.debug) {
            this.log("[%s] was loaded from your eWeLink account and will be refreshed.", accessory.displayName);
         }
         ...
         // accessory.context.reachableLAN = this.lanDevices[device.deviceid] || false;
         ...
         for (let i = 0; i <= accessory.context.channelCount; i++) {
            ...
            try {
               if (!this.devicesInHB.has(device.deviceid + "SW" + i)) {
                  if ([1, 2, 3, 4].includes(i) && this.customHideChanFromHB.includes(device.deviceid + "SW" + i) && accessory.context.type === "switch") {
                     continue;
                  } else {
                     this.addAccessory(device, device.deviceid + "SW" + i, "switch");
                  }
               }
               otherAccessory = this.devicesInHB.get(device.deviceid + "SW" + i);
               if ([1, 2, 3, 4].includes(i) && this.customHideChanFromHB.includes(device.deviceid + "SW" + i) && accessory.context.type === "switch") {
                  this.removeAccessory(otherAccessory);
                  continue;
               }
               ...
               // otherAccessory.context.reachableLAN = this.lanDevices[device.deviceid] || false;
               this.devicesInHB.set(otherAccessory.context.hbDeviceId, otherAccessory);
               this.api.updatePlatformAccessories("homebridge-ewelink-sonoff", "eWeLink", [otherAccessory]);
            } catch (e) {}
         }
      } else {
   ...
   addAccessory(device, hbDeviceId, service) {
      ...
      try {
         ...
         case "bridge":
            let b;
            if ((b = accessory.context.sensorType = this.customBridgeSensors.get(hbDeviceId))) {
   ...
   configureAccessory(accessory) {
      //*** NOTE ***\\
      //*** The return below skips this function being called by Homebridge on restart.
      //*** This is needed as we are not connected to the web socket yet.
      //*** The function is called again when the plugin finishes loading as at that point the web socket is connected.
      if (!this.log) {
         return;
      }
      try {
   ...
   externalDeviceUpdate(device) {
      ...
      if (device.action === "sysmsg") {
            ...
            for (let i = 0; i <= accessory.context.channelCount; i++) {
               try {
                  if (this.devicesInHB.has(device.deviceid + "SW" + i)) {
                     ...
                     this.log("[%s] has been reported [%s].", otherAccessory.displayName, otherAccessory.context.reachableWAN ? "online" : "offline");
                     this.devicesInHB.set(otherAccessory.context.hbDeviceId, otherAccessory);
                     this.api.updatePlatformAccessories("homebridge-ewelink-sonoff", "eWeLink", [otherAccessory]);
                  }
               } catch (e) {
                  this.log.warn("[%s] new status could not be updated - [%s].", otherAccessory.displayName, e);
               }
            }
         }
         ...
         if (this.devicesInHB.has(device.deviceid + "SWX") || this.devicesInHB.has(device.deviceid + "SW0")) {
            ...
            if (this.debug) {
               this.log("[%s] was externally updated will be refreshed.", accessory.displayName);
            }
            if (this.refreshAccessory(accessory, device.params)) {
               return;
            }
            ...
            this.log.warn("[%s] Accessory received via web socket has been hidden from Homebridge as per use configuration.", device.deviceid);
         } else {
            this.log.warn("[%s] Accessory received via web socket does not exist in Homebridge. If it's a new accessory please restart Homebridge so it is added.", device.deviceid);
         }
      } else if (this.debug) {
         this.log.warn("[%s] unknown action or parameters received via web socket.", device.name);
      }
   }
   ...
   removeAllAccessories() {
      try {
         this.warn("[0] primary devices were loaded from your eWeLink account so any eWeLink devices in the Homebridge cache will be removed. This plugin will no longer load as there is no reason to continue.");
   ...
   internalBlindUpdate(accessory, value, callback) {
      try {
         ...
         if (sensorDefinition && !(sensorAccessory = this.devicesInHB.get(blindConfig.sensorFullId))) {
         ...
         if (cPos === value) {
            this.log("[%s] is already [%s]. Nothing to do.", accessory.displayName, value === 0 ? "closed" : "open");
            //   0 = blind closed
            // 100 = blind open
            callback();
            return;
         ...
         this.wsClient.sendUpdate(payload, function () {
            return;
         });
         service
   ...
   internalGarageUpdate(accessory, value, callback) {
      try {
         ...
         let sensorAccessory = false;
         if (sensorDefinition && !(sensorAccessory = this.devicesInHB.get(garageConfig.sensorFullId))) {
            throw "defined sensor doesn't exist";
         }
         let cPos;
         if (sensorAccessory) {
         ...
         this.log("[%s] updating garage door to [%s].", accessory.displayName, value === 0 ? "opening" : "closing");
         this.wsClient.sendUpdate(payload, function () {
            return;
         });
         service
   ...
   internalFanUpdate(accessory, type, value, callback) {
      try {
         ...
         case "power":
            newPower = value;
            newSpeed = value ? 33 : 0;
            newLight = accessory.getService(Service.Lightbulb)
   ...
   internalLightbulbUpdate(accessory, value, callback) {
      try {
         ...
         case "0":
            ...
            for (let i = 1; i <= 4; i++) {
               if (this.devicesInHB.has(accessory.context.eweDeviceId + "SW" + i)) {
                  otherAccessory = this.devicesInHB.get(accessory.context.eweDeviceId + "SW" + i);
         ...
         case "4":
            ...
            otherAccessory = this.devicesInHB.get(accessory.context.eweDeviceId + "SW0");
      ...
      try {
         ...
         for (let i = 1; i <= accessory.context.channelCount; i++) {
            if (this.devicesInHB.has(idToCheck + i)) {
               let otherAccessory = this.devicesInHB.get(idToCheck + i);
            ...
            if (this.devicesInHB.has(idToCheck + i)) {
               ...
               if (params.hasOwnProperty("rfTrig" + (i - 1))) {
                  ...
                  if (timeDifference < this.sensorTimeDifference) {
                     ...
                     master = true;
                     if (this.debug) {
                        this.log("[%s] has detected [%s].", otherAccessory.displayName, otherAccessory.context.sensorType);
                     }
                  }
               }
            }
