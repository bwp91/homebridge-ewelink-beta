...
class eWeLink {
      ...
      if ((accessory = this.devicesInHB.get(device.deviceid + "SWX"))) {
         ...
         for (let i = 0; i <= accessory.context.channelCount; i++) {
            ...
            try {
               if (!this.devicesInHB.has(device.deviceid + "SW" + i)) {
                  if (i > 0 && constants.devicesHideable.includes(accessory.context.type)) {
                     if (this.customHideChanFromHB.includes(device.deviceid + "SW" + i)) {
                        continue;
                     } else {
                        this.addAccessory(device, device.deviceid + "SW" + i, "switch");
                     }
               ...
               if (i > 0 && isRf) {
                  ...
                  if ((ct !== oAccessory.context.sensorType && oAccessory.context.sensorType !== "button") || typeof accessory.context.updated === "undefined") {
                     rfCh = true;
                  }
               }
         ...
         if (rfCh) {
            ...
            try {
               ...
               this.log.warn("[%s] bridge reset failed - [%s]", oAccessory.displayName, err);
            }
            return;
         }
      } else {
   ...
   addAccessory(device, hbDeviceId, type) {
      let channelCount = type === "rf_pri" ? Object.keys(device.tags.zyx_info).length : constants.chansFromUiid[device.extra.uiid],
      ...
      try {
         ...
         case "zb_pri":
            break;
         case "rf_sub":
            accessory.context.rfGroupIndex = parseInt(switchNumber) - 1;
            accessory.context.rfDeviceCount = device.tags.zyx_info[accessory.context.rfGroupIndex].buttonName.length;
            accessory.context.rfChls = {};
            switch (device.tags.zyx_info[accessory.context.rfGroupIndex].remote_type) {
            case "1":
            case "2":
            case "3":
            case "4":
            ...
            device.tags.zyx_info[accessory.context.rfGroupIndex].buttonName.forEach(button => {
               let rfChan, name;
               ...
               this.api.updatePlatformAccessories(constants.packageName, "eWeLink", [mAccessory]);
            });
            break;
         case "zb_sub":
      ...
      try {
         ...
         case "rf_sub":
            ...
            if (accessory.context.sensorType === "button") {
               Object.keys(accessory.context.rfChls).forEach(k => {
                  accessory.getService(Service.Switch, accessory.context.rfChls[k]).getCharacteristic(Characteristic.On)
                     .on("set", (value, callback) => {
                        this.internalRFDeviceUpdate(accessory, accessory.context.rfChls[k], k, true, callback);
                     });
               });
            }
            break;
   ...
   internalRFDeviceUpdate(accessory, name, rfChl, value, callback) {
      try {
         if (!accessory.context.reachableWAN && !accessory.context.reachableLAN) {
            throw "it is currently offline";
         }
         ...
         if (this.debug) {
            this.log("[%s %s] mimicking RF button press.", accessory.displayName, name);
         }
         accessory.getService(name).updateCharacteristic(Characteristic.On, true);
         this.sendDeviceUpdate(accessory, params, callback);
         setTimeout(() => {
      ...
      try {
         ...
         if (params.hasOwnProperty("cmd") && params.cmd === "transmit" && params.hasOwnProperty("rfChl")) { // RF Button
            ...
            if ((bAccessory = this.devicesInHB.get(idToCheck + accessory.context.rfChlMap[params.rfChl]))) {
               let serviceName = bAccessory.context.rfChls[params.rfChl];
               bAccessory.getService(serviceName).updateCharacteristic(Characteristic.On, 1);
               setTimeout(() => {
                  bAccessory.getService(serviceName).updateCharacteristic(Characteristic.On, 0);
               }, 500);
            } else {
