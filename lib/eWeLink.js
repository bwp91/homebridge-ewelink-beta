            ...
            .then(res => { //*** Use the HTTP API host to log into eWeLink ***\\
               ...
               (() => {
                  ...
                  this.httpDevices.forEach(device => {
                     ...
                     // @DEV
                     /* if ([66, 1000, 1770, 2026, 3026].includes(device.extra.uiid)) {
                        this.log.error("----------------------");
                        this.log.warn(JSON.stringify(device.params, null, 2));
                        this.log.error("----------------------");
                     } */
                     // @/DEV
                  });
   ...
   addAccessory(device, hbDeviceId, type) {
      ...
      try {
         ...
         case "zb_sub":
            ...
            case 1000:
               accessory.addService(Service.Switch, "Short Click", "short");
               accessory.addService(Service.Switch, "Double Click", "double");
               accessory.addService(Service.Switch, "Long Click", "long");
               break;
            case 1770:
               accessory.addService(Service.TemperatureSensor);
               accessory.addService(Service.HumiditySensor);
      ...
      try {
         ...
         case 1000:
            if (params.hasOwnProperty("key")) {
               accessory.getService("Short Click").updateCharacteristic(Characteristic.On, params.key === 0);
               accessory.getService("Double Click").updateCharacteristic(Characteristic.On, params.key === 1);
               accessory.getService("Long Click").updateCharacteristic(Characteristic.On, params.key === 2);
               setTimeout(() => {
                  accessory.getService("Short Click").updateCharacteristic(Characteristic.On, false);
                  accessory.getService("Double Click").updateCharacteristic(Characteristic.On, false);
                  accessory.getService("Long Click").updateCharacteristic(Characteristic.On, false);
               }, 2000);
            }
            break;
         case 1770:
