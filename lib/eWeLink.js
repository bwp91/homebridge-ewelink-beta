...
class eWeLink {
   ...
   initialiseDevice(device) {
      ...
      if ((accessory = this.devicesInHB.get(device.deviceid + "SWX"))) {
         if (this.debug) {
            this.log("[%s] was found in eWeLink and will be refreshed.", accessory.displayName);
         }
         ...
         for (let i = 0; i <= accessory.context.channelCount; i++) {
            ...
            try {
               ...
               oAccessory.context.reachableLAN = this.lanDevices[device.deviceid] || false;
   ...
   externalDeviceUpdate(device) {
      ...
      if (device.action === "sysmsg") {
         ...
         if (this.devicesInHB.has(device.deviceid + "SWX") || this.devicesInHB.has(device.deviceid + "SW0")) {
            ...
            this.log.warn("[%s] Accessory received via web socket has been hidden from Homebridge as per user configuration.", device.deviceid);
         } else {
   ...
   internalGarageUpdate(accessory, value, callback) {
      try {
         ...
         if (sensorDefinition && !(sensorAccessory = this.devicesInHB.get(garageConfig.sensorId + "SWX"))) {
            throw "defined sensor doesn't exist";
         }
         let cPos;
         if (sensorAccessory) {
            //*** 1 == contact not detected == garage open || 0 == contact detected == garage closed ***\\
            cPos = sensorAccessory.getService(Service.ContactSensor).getCharacteristic(Characteristic.ContactSensorState).value === 0 ? 1 : 0;
         } else {
            //*** 0 == contact not detected == garage open || 1 == contact detected == garage closed ***\\
   ...
   externalBlindUpdate(accessory, params) {
      try {
         ...
         if (blindConfig.setup === "twoSwitch") {
            ...
            currentState = accessory.getService(Service.WindowCovering).getCharacteristic(Characteristic.CurrentPosition).value;
         }
   ...
   externalGarageUpdate(accessory, params) {
      try {
         ...
         this.log("EGU1: user config looks good so start procedure...");
         let currentPos;
         if (sAccessory) {
            this.log("EGU2: sensor [%s] will be used to determine initial garage position...", sAccessory.displayName);
            ...
            this.log("EGU3: sensor reports a value of [%s] which corresponds to [%s].", currentPos, temp_textString);
            // 1 = contact not detected  -> garage open
            // 0 = contact detected      -> garage closed
            this.log("EGU4: nothing else to do here as will wait for an update from the sensor.", currentPos, temp_textString);
