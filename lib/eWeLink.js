      ...
      try {
         ...
         case "rf_sub":
            ...
            if (accessory.context.sensorType === "button") {
               Object.keys(accessory.context.rfChls).forEach(rfCh => {
                  accessory.getService(accessory.context.rfChls[rfCh]).updateCharacteristic(Characteristic.On, false);
                  accessory.getService(accessory.context.rfChls[rfCh]).getCharacteristic(Characteristic.On)
                     .on("set", (value, callback) => {
                        value ?
                           this.internalRFDeviceUpdate(accessory, rfCh, callback) :
                           callback();
                     });
               });
            }
