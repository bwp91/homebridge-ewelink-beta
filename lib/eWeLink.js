...
class eWeLink {
   ...
   initialiseDevice(device) {
      ...
      if (!this.devicesInHB.has(device.deviceid + "SWX") && !this.devicesInHB.has(device.deviceid + "SW0")) {
         if (device.extra.uiid === 2 && device.brandName === "coolkit" && device.productModel === "0285") {
            ...
            this.addAccessory(device, device.deviceid + "SW0", "zb_pri");
            let i = 1;
            device.params.subDevices.forEach(subDevice => {
               this.addAccessory(device, device.deviceid + "SW" + i, "zb_sub", subDevice);
               i++;
            });
         } else {
   ...
   addAccessory(device, hbDeviceId, type, subDevice = {}) {
      let channelCount = type === "bridge" ? Object.keys(device.params.rfList).length : constants.chansFromUiid[device.extra.uiid],
      ...
      try {
         ...
         case "rf_sub":
            ...
            break;
         case "zb_sub":
            accessory.context.zbDeviceId = subDevice.subDevId; // @TODO check this
            switch (subDevice.uiid) {
            case "1000":
               accessory.context.zbType = "switch";
               accessory.addService(Service.Switch);
               break;
            case "1770":
               accessory.context.zbType = "sensorTempHum";
               accessory.addService(Service.TemperatureSensor);
               accessory.addService(Service.HumiditySensor);
               break;
            case "2026":
               accessory.context.zbType = "sensorMotion";
               accessory.addService(Service.MotionSensor);
               break;
            case "3026":
               accessory.context.zbType = "sensorContact";
               accessory.addService(Service.ContactSensor);
               break;
            default:
               throw "unsupported zigbee device type with uiid [" + subDevice.uiid + "]";
            }
            break;
         default:
   ...
   refreshAccessory(accessory, newParams) {
      ...
      case "switch":
         ...
         return true;
      case "rf_pri":
         this.externalRFBridgeUpdate(accessory, newParams);
         return true;
      case "zb_pri":
         this.externalZBBridgeUpdate(accessory, newParams);
         return true;
      case "zb_sub":
      case "rf_sub":
         return true;
   ...
   externalRFBridgeUpdate(accessory, params) {
      try {
         let idToCheck = accessory.context.hbDeviceId.slice(0, -1),
            timeNow = new Date();
         for (let i = 1; i <= accessory.context.channelCount; i++) {
   ...
   externalZBBridgeUpdate(accessory, params) {
      try {
         // @TODO
      } catch (err) {
         this.log.warn("[%s] could not be updated - [%s].", accessory.displayName, err);
