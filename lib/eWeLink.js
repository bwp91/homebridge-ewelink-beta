...
class eWeLink {
   ...
   internalGarageUpdate(accessory, value, callback) {
      try {
         ...
         this.log("[%s] updating - [%s] garage door.", accessory.displayName, value === 0 ? "opening" : "closing");
         this.wsClient.sendUpdate(payload, function () {
            return;
         });
         service
         ...
         if (!sAccessory) {
            ...
            this.log("[%s] sensor defined so now will wait for the sensor to send update.", accessory.displayName);
         }
      } catch (err) {
   ...
   externalGarageUpdate(accessory, params) {
      try {
         ...
         if (sAccessory) {
            ...
            this.log("EGU4: nothing else to do here as will wait for sensor notification and update garage door accordingly.");
