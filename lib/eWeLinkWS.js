...
module.exports = class eWeLinkWS {
   ...
   login() {
      ...
      this.ws.on("message", m => {
         ...
         this.log.warn(JSON.stringify(device, null, 2));
   ...
   sendUpdate(json, callback) {
      ...
      let sendOperation = req => {
         ...
         if (this.ws) {
            ...
            this.ws.send(req);
            if (this.debugReqRes) {
               let msg = JSON.stringify(json, null, 2).replace(json.apikey, "**hidden**").replace(this.apiKey, "**hidden**").replace(json.deviceid, "**hidden**");
