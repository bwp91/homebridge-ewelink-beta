...
module.exports = class eWeLinkWS {
   ...
   login() {
      ...
      this.wsp.open();
      this.wsp.onOpen.addListener(() => this.wsOpen());
      this.wsp.onUnpackedMessage.addListener(device => this.wsMessage(device));
      this.wsp.onClose.addListener(err => this.wsClose(err));
      this.wsp.onError.addListener(err => this.wsError(err));
   }
   wsOpen() {
      this.wsIsOpen = true;
   ...
   wsClose(err) {
      this.log.warn("Web socket closed - [%s].", err);
      if (err.code !== 1000) {
   ...
   wsError(err) {
      this.log.error("Web socket error - [%s].", err);
      ...
      let sendOperation = req => {
         ...
         if (this.wsp) {
            ...
            this.wsp.sendRequest(req, {
               ...
               case 504:
               default:
                  if (device.hasOwnProperty("reason") && device.reason === "Request Timeout") {
                     callback();
                     return true;
                  }
