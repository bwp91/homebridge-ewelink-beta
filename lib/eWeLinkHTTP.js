...
module.exports = class eWeLinkHTTP {
   ...
   login() {
      ...
      return new Promise((resolve, reject) => {
         axios({
            url: "https://" + this.httpHost + "/api/user/login", // ***** CHANGE API HOST HERE
            method: "post",
            headers: {
            ...
            this.log.warn(err);
            reject("An error occurred whilst logging in. Please see Homebridge log.");
         });
      });
   }
   getHost() {
      ...
      return new Promise((resolve, reject) => {
         axios.get("https://api.coolkit.cc:8080/api/user/region", {
            ...
            this.log.error("** Could not load homebridge-ewelink-sonoff **");
            this.log.warn(err);
            reject("An error occurred whilst getting HTTP host. Please see Homebridge log.");
         });
      });
   }
   getDevices() {
      return new Promise((resolve, reject) => {
         axios.get("https://" + this.httpHost + "/api/user/device", {
            ...
            this.log.error("** Could not load homebridge-ewelink-sonoff **");
            this.log.warn(err);
            reject("An error occurred whilst getting obtaining device list. Please see Homebridge log.");
         });
      });
   }
};